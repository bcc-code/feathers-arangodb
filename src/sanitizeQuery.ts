...
sanitizeFieldName(fieldName: string, removeSpaces: boolean = true): string {
  let tempValue = fieldName;
  if(removeSpaces) {
    tempValue = fieldName.split(' ')[0] //we only expect single words here in normal circumstances
  }
